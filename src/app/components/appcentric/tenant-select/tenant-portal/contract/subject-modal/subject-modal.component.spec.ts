...
describe('SubjectModalComponent', () => {
  ...
  it('should have correct required and optional fields by default', () => {
    ...
    const optionalFields = ['alias', 'description', 'applyBothDirections', 'reverseFilterPorts', 'globalAlias'];
  ...
  describe('globalAlias', () => {
    it('should have a maximum length of 100', () => {
      const { globalAlias } = component.form.controls;

      globalAlias.setValue('a');
      expect(globalAlias.valid).toBe(true);

      globalAlias.setValue('a'.repeat(101));
      expect(globalAlias.valid).toBe(false);
    });
  });
});

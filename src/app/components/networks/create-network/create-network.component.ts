...
export class CreateNetworkComponent implements OnInit, ComponentCanDeactivate {
  ...
  calculateNetwork() {
    ...
    if (!this.ipService.ipv4MaskLessThan(this.cidrAddress, 32)) {
      this.cidrAddress = this.ipService.updateIPv4CidrMask(
        this.cidrAddress,
        32,
      );
    }
    ...
    this.usableRange = `${ipv4Range
      .getFirst()
      .nextIPNumber()}-${ipv4Range.getLast().previousIPNumber()}`;
    this.rangeSize = ipv4Range.getSize().toString();
    this.subnet.network = ipv4Range.getFirst().toString();
    this.subnet.subnet_mask = this.ipService.calculateIPv4SubnetMask(
      this.cidrAddress,
    );
  ...
  createNetwork(action: string) {
    ...
    this.automationApiService
      .getSubnets(this.subnet.vrf_group_id)
      .subscribe(data => {
  ...
  private checkNetwork(existingSubnets: Subnet[], action: string) {
    ...
    const checkDuplicateResult = this.ipService.checkIPv4SubnetDuplicate(
      this.subnet,
      this.vlanId,
      existingSubnets,
    );
    ...
    if (checkDuplicateResult[1] !== 'network') {
      const checkOverlapResult = this.ipService.checkIPv4RangeOverlap(
        this.subnet,
        existingSubnets,
      );
  ...
  private launchJobs() {
    ...
    this.automationApiService
      .launchTemplate('save-network', body, true)
      .subscribe(
        data => {
